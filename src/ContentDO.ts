...
export class ContentDO extends DurableObject<Env> {
    ...
    private addCors(response: Response): Response {
    ...
    private sendError(message: string, status = 500, extra: any = {}): Response {
        return this.addCors(Response.json({ success: false, error: message, ...extra }, { status }));
    }
    ...
    async fetch(request: Request): Promise<Response> {
        ...
        if (this.isRateLimited(request)) {
            return this.addCors(new Response('Too many requests', { status: 429 }));
        }
        ...
        if (url.pathname === '/health' || url.pathname === '/stats') {
            response = await this.handleHealthStats(request, url);
        } else if (url.pathname.startsWith('/analytics')) {
            response = await this.handleAnalytics(request, url);
        } else if (url.pathname.startsWith('/notifications')) {
            response = await this.handleNotifications(request, url);
        } else if (url.pathname.startsWith('/telegram/auth')) {
        ...
        } else if (url.pathname === '/sources/rss') {
            response = await this.handleRSS(request, url);
        } else if (url.pathname.startsWith('/webhooks/')) {
            const type = url.pathname.split('/')[2] as any;
            response = await this.handleWebhook(request, type);
        } else if (url.pathname.startsWith('/knowledge')) {
            response = await this.handleKnowledgeSync(request, url);
        } else if (url.pathname.startsWith('/signals')) {
            response = await this.handleSignalSearch(request, url);
        } else if (url.pathname === '/internal/scheduled') {
    ...
    private async handleRSS(request: Request, url: URL): Promise<Response> {
        ...
        if (request.method === 'POST') {
            const { name, url: feedUrl } = await request.json() as any;
            if (!name || !feedUrl) return this.sendError('Missing name or url');
            const id = crypto.randomUUID();
            this.ctx.storage.sql.exec(
                "INSERT INTO channels (id, name, type, feed_url, created_at) VALUES (?, ?, 'rss', ?, ?)",
                id, name, feedUrl, Date.now()
            );
    ...
    private async handleAdmin(request: Request, url: URL): Promise<Response> {
        ...
        if (url.pathname === '/admin/settings') {
            if (request.method === 'GET') {
                const settings = this.ctx.storage.sql.exec("SELECT * FROM settings").toArray();
                const map: any = {};
            ...
            if (request.method === 'POST') {
                ...
                if (!key) return this.sendError('Missing key');
                this.ctx.storage.sql.exec("INSERT OR REPLACE INTO settings (key, value, updated_at) VALUES (?, ?, ?)", key, JSON.stringify(value), Date.now());
                return Response.json({ success: true });
            }
        }
    ...
    private async scrubSensitiveContent(text: string): Promise<string | null> {
        if (!text) return text;
        // 1. Redact API Keys (Simple Patterns)
        text = text.replace(/sk-[a-zA-Z0-9]{32,}/g, '[REDACTED_API_KEY]');
        text = text.replace(/ghp_[a-zA-Z0-9]{30,}/g, '[REDACTED_API_KEY]');
        text = text.replace(/xox[baprs]-[a-zA-Z0-9-]+/g, '[REDACTED_API_KEY]');
            ...
            if (Array.isArray(keywords)) {
                for (const word of keywords) {
                    if (new RegExp(`\\b${word}\\b`, 'i').test(text)) {
                        console.log(`[ContentRefinery] Filtered sensitive content: ${word}`);
                        return null;
                    }
                }
            }
    ...
    private async handleIngestInternal(body: any): Promise<string | null> {
        // Prevent feedback loop from output channel
        if (body.title && /high\s*signal\s*alpha/i.test(body.title)) {
            console.log('[ContentRefinery] Skipping ingestion from output channel:', body.title);
            return null;
        }
        ...
        const scrubbed = await this.scrubSensitiveContent(text);
