...
import { Activity, Shield, Cpu, Zap, Search, Bell, TrendingUp, MessageCircle, X, Phone, Key, Lock, QrCode, Smartphone, Settings, Plus, Trash2, Rss, Hash, Globe, Share2, BookOpen, Clock, ArrowLeft, ChevronRight } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const enableNotifications = async () => {
    ...
    try {
      ...
      await fetch(`${API_BASE}/notifications/subscribe`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(sub) });
      setSubscribed(true);
            ...
            <div className="lg:col-span-2 flex flex-col gap-6 overflow-hidden">
              <div className="flex justify-between items-center">
                <h2 className="flex items-center gap-2 font-semibold">
                  ...
                  {historyMode ? 'Historical Archive' : 'Intelligence Feed'}
                </h2>
                <div className="flex items-center gap-2 text-[10px] font-mono text-white/40 uppercase tracking-widest">
                  <Clock className="w-3 h-3 text-success animate-pulse" />
                  Live Sync
                </div>
              </div>
